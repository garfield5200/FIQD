...
for item in dataset:
    ...
    pred = tokenize(full_text)
    gt = tokenize(answer)

    stats[label]["preds"].append(pred)
    stats[label]["gts"].append(gt)
...
for label in stats:
    ...
    for i in range(len(preds)):
        all_preds[len(all_preds)] = [preds[i]]
        all_gts[len(all_gts)] = [gts[i]]

overall_cider, _ = cider_scorer.compute_score(all_gts, all_preds)

overall_css = np.mean(
    [css for label in stats for css in stats[label]["css"]]
)

results["overall"] = {
    "CIDEr": float(f"{overall_cider:.3f}"),
    "CSS": float(f"{overall_css:.3f}")
}
# =============================
# 保存结果
# =============================
